        ...
        <div class="p-6 bg-white border-b border-gray-200">
          ...
          <form method="POST" action="{{ route('admin.items.update', ['item' => $item->item_id ]) }}" enctype="multipart/form-data" class="grid gap-4 grid-cols-1 md:grid-cols-2 lg:px-24">
            ...
            <div class="grid gap-4 grid-cols-1">
              ...
              <div>
                <x-label>商品名</x-label>
                <x-input type="text" name="item_name" class="w-full" value="{{ old('item_name', $item->item_name) }}" />
              </div>
              <!-- 単価 -->
              <div>
                <x-label>単価</x-label>
                <x-input type="number" name="unit_price" class="w-1/2" value="{{ old('unit_price', $item->unit_price) }}" />
              </div>
              <!-- 商品分類 -->
              <div>
                ...
                <select name="item_category" class="w-1/2 rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" >
                  @foreach(App\Enums\ItemCategory::asSelectArray() as $key => $value)
                    <option value="{{$key}}" @if(old('item_category', $item->item_category) == $key) selected @endif>{{ $value }}</option>
                  @endforeach
                </select>
              </div>
            </div>
            <div>
              ...
              <img class="w-80 h-80" src="{{ asset(isset($item->item_image_path) ? '/storage/' . $item->item_image_path : '/img/noimage.png') }}">
            </div>
