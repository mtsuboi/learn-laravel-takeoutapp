      ...
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
          <div class="py-3 flex justify-between">
            <form class="flex items-center" mothod="GET" action="{{ route('items.create') }}">
              <x-button>
            ...
            <form class="flex items-center" mothod="GET" action="{{ route('items.index') }}">
              ...
              @foreach($items as $item)
                <tr>
                  ...
                  <td class="px-4 py-3 border-t-2 border-gray-200">
                    <a class="no-underline hover:underline text-blue-500" href="{{ route('items.edit', ['item' => $item->item_id]) }}">{{$item->item_name}}</a>
                  </td>
                  ...
                  <td class="w-10 px-4 py-3 border-t-2 border-gray-200">
                    <form method="POST" action="{{ route('items.destroy', ['item' => $item->item_id]) }}" onsubmit="return confirm('削除してよろしいですか？')">
                      @method('DELETE')
                      @csrf
